...
namespace Academy.Infrastructure.BackgroundJobs
{
    internal static class BackgroundJobsServiceRegistration
    {
        private static readonly ILogger _logger = Log.ForContext(typeof(BackgroundJobsServiceRegistration));

        internal static IServiceCollection AddBackgroundJobs(this IServiceCollection services, IConfiguration config)
        {
            var isEnableHangfire = config.GetSection("HangfireSettings:Enable").Get<bool>();
            if (isEnableHangfire)
            {
                ...
                services.AddSingleton<Hangfire.JobActivator, JobActivator>();
                services.AddHangfire((provider, hangfireConfig) =>
                {
                    hangfireConfig.UseDatabase(storageSettings.StorageProvider, storageSettings.ConnectionString, config);
                    hangfireConfig.UseFilter(new JobFilter(provider));
                    hangfireConfig.UseFilter(new LogJobFilter());
