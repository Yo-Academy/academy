...
namespace Academy.Infrastructure.Storage.AWS
{
    public class BlobStream : Stream
    {
        ...
        private void Flush(bool disposing)
        {
            ...
            {
                _metadata.UploadId = _s3.InitiateMultipartUploadAsync(new Amazon.S3.Model.InitiateMultipartUploadRequest()
                {
        ...
        private void CompleteUpload()
        {
            ...
            if (Length > 0)
            {
                _s3.CompleteMultipartUploadAsync(new Amazon.S3.Model.CompleteMultipartUploadRequest()
                {
                    BucketName = _metadata.BucketName,
                    Key = _metadata.Key,
