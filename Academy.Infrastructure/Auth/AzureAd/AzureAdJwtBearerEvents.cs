...
namespace Academy.Infrastructure.Auth.AzureAd
{
    internal class AzureAdJwtBearerEvents : JwtBearerEvents
    {
        ...
        public AzureAdJwtBearerEvents(ILogger logger, IConfiguration config) => (_logger, _config) = (logger, config);
        ...
        public override async Task TokenValidated(TokenValidatedContext context)
        {
            ...
            var tenant = issuer == _config["SecuritySettings:AzureAd:RootIssuer"]
                ? await tenantDb.TenantInfo.FindAsync(_config.GetSection(nameof(DefaultTenantSettings)).Get<DefaultTenantSettings>()!.Id)
            ...
            // Adding tenant claim.
            identity.AddClaim(new Claim(Claims.Tenant, tenant.Id));
